...
FEAS_BOUNDARY_V1_WORKLOAD: List[Tuple[str, int, float]] = [
    # Premium tier：3 个，ci=1.3（终局甜点：P1 需求 ~252 Gbps）
    ("LLaMA-2-13B", 8, 1.3),       # P1, contested ✓
    ("LLaMA-2-7B", 8, 1.3),        # P2, contested ✓
    ("BERT-Large-fp16", 2, 1.3),    # P3, non-contested
    # Standard tier：4 个，ci=2.0（S1/S2 需求 ~165 Gbps）
    ("LLaMA-2-13B", 8, 2.0),       # S1, contested ✓
    ("T5-11B-fp16", 8, 2.0),       # S2, contested ✓
    ("BERT-Large-fp16", 4, 2.0),    # S3, non-contested
    ("ViT-Base", 2, 2.0),           # S4, non-contested
]

# feas_boundary_v2: ci=2.0/3.0（真边界）
# - Σattain_bw = 879 Gbps < 1000G（可达）
# - premium 全达标（665 Gbps < 1000G）
# - standard 有界降级（剩余 335G → sas≈0.67）
FEAS_BOUNDARY_V2_WORKLOAD: List[Tuple[str, int, float]] = [
    # Premium tier：3 个，ci=2.0
    ("LLaMA-2-13B", 8, 2.0),       # P1, contested
    ("LLaMA-2-7B", 8, 2.0),        # P2, contested
    ("BERT-Large-fp16", 2, 2.0),   # P3, non-contested
    # Standard tier：4 个，ci=3.0
    ("LLaMA-2-13B", 8, 3.0),       # S1, contested
    ("T5-11B-fp16", 8, 3.0),       # S2, contested
    ("BERT-Large-fp16", 4, 3.0),   # S3, non-contested
    ("ViT-Base", 2, 3.0),          # S4, non-contested
]

# feas_boundary_v3 E1 正相关场景（定量定标，14 jobs: 8P+6S）：
# - Premium: 8 jobs (LLaMA-13B×4 ci=1.5, LLaMA-7B×2 ci=1.5, BERT dp4 ci=2.0, T5-11B ci=2.0)
# - Standard: 6 jobs (LLaMA-13B×2 ci=3.0, BERT dp4×2 ci=3.0, BERT dp2×2 ci=3.0)
# - ΣP=605.7G, ΣS=263.6G, C*=737.5G
# - per_flow_attain 全部 ≤100G（BERT dp2 ci=3.0: 41.7G/flow）
FEAS_BOUNDARY_V3_WORKLOAD: List[Tuple[str, int, float]] = [
    # Premium tier: 8 jobs, ci=1.5/2.0（大模型紧ci——正相关）
    ("LLaMA-2-13B", 8, 1.5),       # J0
    ("LLaMA-2-13B", 8, 1.5),       # J1
    ("LLaMA-2-13B", 8, 1.5),       # J2
    ("LLaMA-2-13B", 8, 1.5),       # J3
    ("LLaMA-2-7B", 8, 1.5),        # J4
    ("LLaMA-2-7B", 8, 1.5),        # J5
    ("BERT-Large-fp16", 4, 2.0),   # J6
    ("T5-11B-fp16", 8, 2.0),       # J7
    # Standard tier: 6 jobs, ci=3.0（小/中模型松ci——正相关）
    ("LLaMA-2-13B", 8, 3.0),       # J8
    ("LLaMA-2-13B", 8, 3.0),       # J9
    ("BERT-Large-fp16", 4, 3.0),   # J10
    ("BERT-Large-fp16", 4, 3.0),   # J11
    ("BERT-Large-fp16", 2, 3.0),   # J12
    ("BERT-Large-fp16", 2, 3.0),   # J13
]

# feas_boundary_v3 E2-pro CRUX 对照组（原 v3-anti，13 jobs: 5P+8S）：
# CRUX 主场：premium=小模型高 intensity → CRUX 高优先级
# - Premium: 5 jobs (BERT dp2×2 ci=1.5, BERT dp4 ci=2.0, ViT-Large dp2×2 ci=1.5) —— 小模型紧ci
# - Standard: 8 jobs (LLaMA-13B×3 ci=3.0, LLaMA-7B×3 ci=3.0, T5-11B×2 ci=3.0) —— 大模型松ci
# - ΣP=632.9G, ΣS=275.4G, C*=770.6G
# - CRUX P-attn 预期高（≈100%），v4 与 CRUX 持平
FEAS_BOUNDARY_V3_PRO_WORKLOAD: List[Tuple[str, int, float]] = [
    # Premium tier: 5 jobs, ci=1.5/2.0（小模型紧ci → CRUX 高 intensity → 高优先级）
    ("BERT-Large-fp16", 2, 1.5),   # J0  intensity=0.92 → CRUX HIGH
    ("BERT-Large-fp16", 2, 1.5),   # J1  intensity=0.92
    ("BERT-Large-fp16", 4, 2.0),   # J2  intensity=1.84
    ("ViT-Large", 2, 1.5),         # J3  intensity=1.22
    ("ViT-Large", 2, 1.5),         # J4  intensity=1.22
    # Standard tier: 8 jobs, ci=3.0（大模型松ci → CRUX 低 intensity → 低优先级）
    ("LLaMA-2-13B", 8, 3.0),       # J5  intensity=0.15 → CRUX LOW
    ("LLaMA-2-13B", 8, 3.0),       # J6
    ("LLaMA-2-13B", 8, 3.0),       # J7
    ("LLaMA-2-7B", 8, 3.0),        # J8  intensity=0.14
    ("LLaMA-2-7B", 8, 3.0),        # J9
    ("LLaMA-2-7B", 8, 3.0),        # J10
    ("T5-11B-fp16", 8, 3.0),       # J11 intensity=0.14
    ("T5-11B-fp16", 8, 3.0),       # J12 intensity=0.14
]

# feas_boundary_v3 E2' 真·杀场（14 jobs: 9P+5S）：
# CRUX 杀场：premium=大模型低 intensity → CRUX 压底，standard=小模型高 intensity → CRUX 抬顶
# - Premium: 9 jobs, ci=1.5/2.0（大模型紧ci → CRUX 低 intensity → 低优先级）
# - Standard: 5 jobs, ci=3.0（小模型松ci → CRUX 高 intensity → 高优先级）
# - ΣP=623.5G, ΣS=257.8G, C*=752.4G
# - CRUX P-attn 预期崩溃（intensity排序与tier完全反向）
FEAS_BOUNDARY_V3_PRIME_WORKLOAD: List[Tuple[str, int, float]] = [
    # Premium tier: 9 jobs, ci=1.5/2.0（大模型紧ci → CRUX 低 intensity → 被压底）
    ("LLaMA-2-13B", 8, 1.5),       # J0  intensity=0.15 → CRUX LOW ★ STARVED
    ("LLaMA-2-13B", 8, 1.5),       # J1  intensity=0.15
    ("LLaMA-2-13B", 8, 1.5),       # J2
    ("LLaMA-2-13B", 8, 1.5),       # J3
    ("LLaMA-2-13B", 8, 1.5),       # J4
    ("LLaMA-2-13B", 8, 1.5),       # J5
    ("LLaMA-2-7B", 8, 1.5),        # J6  intensity=0.14
    ("LLaMA-2-7B", 8, 1.5),        # J7
    ("T5-11B-fp16", 8, 2.0),       # J8  intensity=0.14
    # Standard tier: 5 jobs, ci=3.0（小模型松ci → CRUX 高 intensity → 抬顶）
    ("BERT-Large-fp16", 4, 3.0),   # J9  intensity=1.84 → CRUX HIGH ★ PRIORITY
    ("BERT-Large-fp16", 4, 3.0),   # J10 intensity=1.84
    ("BERT-Large-fp16", 4, 3.0),   # J11 intensity=1.84
    ("ViT-Large", 2, 3.0),         # J12 intensity=1.22
    ("ViT-Large", 2, 3.0),         # J13 intensity=1.22
]
