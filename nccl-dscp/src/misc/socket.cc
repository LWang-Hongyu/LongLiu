...
static ncclResult_t socketWait(int op, struct ncclSocket* sock, void* ptr, int size, int* offset) {
  ...
  while (*offset < size) {
    ...
    // 如果offset没有增加，说明socket暂时不可用（非阻塞模式），需要适当等待
    if (*offset == prevOffset && *offset < size) {
      if (retryCount++ > 100000) {
        WARN("socketWait: too many retries without progress, offset=%d size=%d", *offset, size);
        return ncclSystemError;
      }
      // 每隔一段时间使用微秒级延迟避免CPU忙等待
      if (retryCount % 10000 == 0) {
        usleep(SLEEP_INT);
      }
    } else {
      retryCount = 0; // 重置计数器，因为有进展
    }
    prevOffset = *offset;
  }
  return ncclSuccess;
