...
用法：
    ...
    python experiments/exp_e10_wfs.py --seeds 2 --quick   # 快速验证
"""
...
E10_CONFIG = os.path.join(PROJECT_ROOT, "configs", "e10_wfs.yaml")
...
def load_e10_config() -> dict:
    """读取 configs/e10_wfs.yaml。"""
    with open(E10_CONFIG) as f:
...
def main():
    parser = argparse.ArgumentParser(description="E10 WFS baseline experiment")
    ...
    print("=" * 80)
    print("E10: Weighted Fair Sharing (WFS) Baseline")
    print("=" * 80)
    ...
    # E1 带宽阶梯
    print("--- E1 Bandwidth Ladder ---")
    for spine_bw in cfg["e1_spine_bw_gbps"]:
        for pn in cfg["policies"]:
            seed_vals = []
            for seed in seeds:
                label = f"E1 @{spine_bw}G {pn} s{seed}"
                print(f"[{label}] ", end="", flush=True)
                try:
                    r = run_single("E1", FEAS_BOUNDARY_V3_WORKLOAD, spine_bw, pn, seed, frozen)
    ...
    # 保存 summary CSV
    os.makedirs("outputs/e10_wfs", exist_ok=True)
    with open("outputs/e10_wfs/summary.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["scene", "spine_bw", "policy", "n_seeds", "seeds",
        ...
        for k, runs in sorted(groups.items()):
            ...
            p_caps = [r["p_cap"] for r in runs]
            s_caps = [r["s_cont_cap"] for r in runs]
            writer.writerow([scene, bw, pol, len(runs),
    ...
    print(f"\nSummary saved to outputs/e10_wfs/summary.csv")
    print("*** E10 COMPLETE ***")
