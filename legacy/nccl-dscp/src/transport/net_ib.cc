...
//----------longliu8 add----------
// Global TC value for RDMA QP traffic_class injection.
// Updated by the DSCP adapter when DSCP changes.
// Used as initial TC for new QPs at creation time.
static int g_ncclIbTc = 0;

// Set the default TC value for new QPs.
// Called by the DSCP adapter when DSCP is updated (e.g., at epoch boundaries).
// New QPs created after this call will use the updated TC value.
void ncclIbSetTc(int tc) {
  g_ncclIbTc = tc;
}

// Get the current default TC value for new QPs.
// Falls back to NCCL_IB_TC env var if the global value matches the default.
int ncclIbGetTc(void) {
  // If g_ncclIbTc has been explicitly set by the DSCP adapter, use it.
  if (g_ncclIbTc != 0) return g_ncclIbTc;
  // Otherwise, fall back to NCCL_IB_TC env var.
  return ncclParamIbTc();
}
//----------longliu8 add----------
 ...
 static void* ncclIbAsyncThreadMain(void* args) {
   ...
   static int gid_change_warned = 0;  // 静态计数器，用于跟踪GID_CHANGE是否已打印
   while (1) {
     ...
     if (event.event_type == IBV_EVENT_COMM_EST) {
       // COMM_EST是正常的连接建立事件，不打印
     } else if (event.event_type == IBV_EVENT_GID_CHANGE) {
       // GID_CHANGE在RoCE网络中很常见且通常不影响性能
       // 只在第一次发生时打印INFO级别的消息，后续静默
       if (gid_change_warned++ == 0) {
         INFO(NCCL_NET, "NET/IB : GID table change detected (this is normal for RoCE, suppressing further messages)");
       }
     } else {
       // 其他事件保持WARN级别
...
// (struct ncclIbGidInfo is now in net_ib_types.h)
...
// struct ncclIbRequest, ncclIbVerbs, ncclIbSendFifo, ncclIbSendComm,
// ncclIbGpuFlush, ncclIbRemFifo, ncclIbRecvComm are in net_ib_types.h
...
static_assert((offsetof(struct ncclIbRecvComm, remFifo) % 32) == 0, "ncclIbRecvComm remFifo must be 32-byte aligned");
 ...
 ncclResult_t ncclIbRtrQp(struct ibv_qp* qp, uint32_t qpn, struct ncclIbQpInfo* info) {
  ...
  // Verify traffic_class after ibv_modify_qp
  {
    struct ibv_qp_attr qa;
    struct ibv_qp_init_attr ia;
    memset(&qa, 0, sizeof(qa));
    memset(&ia, 0, sizeof(ia));
    if (wrap_ibv_query_qp(qp, &qa, IBV_QP_AV, &ia) == 0) {
      INFO(NCCL_NET, "NET/IB: QP RTR verify traffic_class=%d sl=%d is_global=%d",
           qa.ah_attr.grh.traffic_class,
           qa.ah_attr.sl,
           qa.ah_attr.is_global);
    }
  }
 ...
 //----------longliu8 add----------
// Dynamically update QP priority (SL and TC) for RDMA data channel
// This function modifies the SL and TC of an existing QP without changing other attributes
ncclResult_t ncclIbUpdateQpPriority(struct ibv_qp* qp, int sl, int tc, uint8_t link_layer) {
  if (qp == NULL) return ncclInvalidArgument;
  ...
  // Query current QP attributes (we need AV attributes and state)
  int attr_mask = IBV_QP_STATE | IBV_QP_AV;
  if (wrap_ibv_query_qp(qp, &qpAttr, attr_mask, &initAttr) != 0) {
    WARN("NET/IB: Failed to query QP for priority update");
    return ncclSystemError;
  }
  
  // Only modify if QP is in RTS state (ready to send)
  if (qpAttr.qp_state != IBV_QPS_RTS) {
    // QP not ready, skip update
    return ncclSuccess;
  }
  
  // Store original values for verification
  int original_sl = qpAttr.ah_attr.sl;
  int original_tc = (link_layer == IBV_LINK_LAYER_ETHERNET && qpAttr.ah_attr.is_global) 
                     ? qpAttr.ah_attr.grh.traffic_class : 0;
  
  // Update SL and TC while preserving other AV attributes
  qpAttr.ah_attr.sl = sl;
  
  // For RoCE (Ethernet link layer), update traffic_class in GRH
  if (link_layer == IBV_LINK_LAYER_ETHERNET && qpAttr.ah_attr.is_global) {
    qpAttr.ah_attr.grh.traffic_class = tc;
  }
  // For InfiniBand, TC is not used (only SL matters)
  // Note: Some IB implementations may support TC via other mechanisms
  
  // Modify QP with updated AV attributes
  // IBV_QP_AV flag allows modifying Address Vector (including SL and TC)
  if (wrap_ibv_modify_qp(qp, &qpAttr, IBV_QP_AV) != 0) {
    WARN("NET/IB: Failed to modify QP priority: SL=%d->%d, TC=%d->%d (link_layer=%d)", 
         original_sl, sl, original_tc, tc, link_layer);
    return ncclSystemError;
  }
  
  // Verify the modification (optional, for debugging)
  // Re-query to confirm the change took effect
  memset(&qpAttr, 0, sizeof(struct ibv_qp_attr));
  if (wrap_ibv_query_qp(qp, &qpAttr, IBV_QP_AV, &initAttr) == 0) {
    int actual_sl = qpAttr.ah_attr.sl;
    int actual_tc = (link_layer == IBV_LINK_LAYER_ETHERNET && qpAttr.ah_attr.is_global) 
                    ? qpAttr.ah_attr.grh.traffic_class : 0;
    
    if (actual_sl != sl || (link_layer == IBV_LINK_LAYER_ETHERNET && actual_tc != tc)) {
      WARN("NET/IB: QP priority modification may not have taken effect: "
           "requested SL=%d/TC=%d, actual SL=%d/TC=%d", sl, tc, actual_sl, actual_tc);
      // Don't fail, as some drivers may not support runtime modification
    }
  }
