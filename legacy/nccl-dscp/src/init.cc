 ...
 ************************************************************************/
 ...
 static ncclResult_t commFree(ncclComm_t comm) {
   ...
   // Save rank before commPoison (which sets rank to -1)
   int savedRank = comm->rank;
   
   //----------longliu8 add----------
   // Export and destroy communication statistics BEFORE commPoison
   // Export on all ranks to capture per-rank statistics
   // Export even if numIterations is 0 to ensure file is created
   if (comm->commStats.enabled) {
     char filename[512];
     // Save to /longliu8/nccl/staticsJson directory
     const char* statsDir = "/longliu8/nccl/staticsJson";
     
     // Create directory if it doesn't exist (safe for all ranks, mkdir is idempotent if exists)
     struct stat st = {0};
     if (stat(statsDir, &st) == -1) {
       // Directory doesn't exist, try to create it
       mkdir(statsDir, 0755);
     }
     
     snprintf(filename, sizeof(filename), "%s/comm_stats_rank%d.json", statsDir, savedRank);
     
     // Always try to export, even if numIterations is 0
     // This ensures the file is created and can be checked
     ncclResult_t exportRet = ncclCommStatsExportToFile(&comm->commStats, filename, comm);
     if (exportRet == ncclSuccess) {
       INFO(NCCL_INIT, "comm %p rank %d: Statistics exported to %s (numIterations=%d)", 
            comm, savedRank, filename, comm->commStats.numIterations);
     } else {
       WARN("comm %p rank %d: Failed to export statistics to %s (error %d)", 
            comm, savedRank, filename, exportRet);
     }
     
     // Note: The main file (comm_stats_rank%d.json) already contains all epochs grouped together.
     // Separate epoch files are disabled by default to avoid file clutter.
     // If you need separate epoch files, set NCCL_STATS_EXPORT_EPOCH_FILES=1
     // (This code is kept for optional use but disabled by default)
     char* export_epoch_files = getenv("NCCL_STATS_EXPORT_EPOCH_FILES");
     int should_export_epoch_files = (export_epoch_files != NULL && atoi(export_epoch_files) == 1);
     
     if (should_export_epoch_files && comm->commStats.numIterations > 0) {
       // Get batches_per_epoch from environment variable (set by training code)
       ...
       // If not set, try auto-detection (same logic as ExportToFile)
       if (batches_per_epoch <= 0 && comm->commStats.numIterations > 0) {
         // Try common batch counts for exact division
         int test_counts[] = {4, 8, 10, 12, 16, 20, 24, 32, 40, 48, 64, 80, 100, 128, 200, 256};
         int num_tests = sizeof(test_counts) / sizeof(test_counts[0]);
         
         for (int t = 0; t < num_tests; t++) {
           int test_batches = test_counts[t];
           if (test_batches > comm->commStats.numIterations) continue;
           
           int test_epochs = (comm->commStats.numIterations + test_batches - 1) / test_batches;
           int remainder = comm->commStats.numIterations % test_batches;
           
           // Prefer exact division
           if (remainder == 0 && test_epochs >= 1 && test_epochs <= 1000) {
             batches_per_epoch = test_batches;
             break;
           }
         }
         
         // If no exact match, use a reasonable estimate
         if (batches_per_epoch <= 0) {
           if (comm->commStats.numIterations <= 100) {
             batches_per_epoch = comm->commStats.numIterations;
           } else {
             // Try powers of 2
             for (int test = 8; test <= 256 && test < comm->commStats.numIterations; test *= 2) {
               if (comm->commStats.numIterations % test == 0) {
             ...
             if (batches_per_epoch <= 0) {
               batches_per_epoch = (comm->commStats.numIterations + 9) / 10;
       ...
       if (batches_per_epoch > 0) {
         int total_epochs = (comm->commStats.numIterations + batches_per_epoch - 1) / batches_per_epoch;
         for (int epoch = 0; epoch < total_epochs; epoch++) {
           int start_iter = epoch * batches_per_epoch;
           int end_iter = (epoch + 1) * batches_per_epoch - 1;
           if (end_iter >= comm->commStats.numIterations) {
             end_iter = comm->commStats.numIterations - 1;
           }
           
           char epoch_filename[512];
           snprintf(epoch_filename, sizeof(epoch_filename), 
                    "%s/comm_stats_rank%d_epoch%d.json", statsDir, savedRank, epoch);
           
           ncclResult_t epochRet = ncclCommStatsExportRangeToFile(&comm->commStats, 
                                                                   epoch_filename,
                                                                   start_iter, end_iter);
           if (epochRet == ncclSuccess) {
             INFO(NCCL_INIT, "comm %p rank %d: Epoch %d statistics exported to %s (iterations %d-%d)", 
                  comm, savedRank, epoch, epoch_filename, start_iter, end_iter);
           }
         }
       }
     }
  }
  ncclCommStatsDestroy(&comm->commStats);
 ...
 static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob* job_) {
   ...
   //----------longliu8 add----------
   // Declare DSCP adapter variables early to avoid goto crossing initialization
   char* dscp_adapter_enabled = NULL;
   double slo_threshold = 1.2; // Default SLO threshold
   char* slo_env = NULL;
   //----------longliu8 add----------
  ...
  if (slo_env) {
    slo_threshold = atof(slo_env);
      ...
      if (dscp_adapter_enabled == NULL || atoi(dscp_adapter_enabled) == 1) {
        ...
        //----------longliu8 add----------
        // Set comm reference for QP updates
        comm->dscpAdapter.comm = comm;
        //----------longliu8 add----------
      } else {
        // Disable if explicitly set to 0
