...
ncclResult_t ncclEnqueueCheck(struct ncclInfo* info) {
  ...
  //----------longliu8 add----------
  // Record communication operation for statistics
  if (info->comm && info->comm->commStats.enabled) {
    int current_iteration = 0;
    char* iter_env = getenv("NCCL_STATS_ITERATION");
    if (iter_env) {
      current_iteration = atoi(iter_env);
    }
    ncclCommStatsStartOp(&info->comm->commStats, info, current_iteration);
  }
  
  // Process DSCP epoch triggers (replaces getenv-based epoch boundary mechanism).
  // ncclDscpEpochStart/End (called by PyTorch via ctypes) set pending flags;
  // this call checks and processes them (end first, then start).
  if (info->comm && info->comm->dscpAdapter.enabled) {
    NCCLCHECKGOTO(ncclDscpAdapterCheckEpochTriggers(&info->comm->dscpAdapter, &info->comm->commStats), ret, fail);
  }
//----------longliu8 add----------
  NCCLCHECKGOTO(taskAppend(info->comm, info), ret, fail);
  //----------longliu8 add----------
  // Record operation end (note: actual end time will be when CUDA stream synchronizes)
  if (info->comm && info->comm->commStats.enabled) {
    int current_iteration = 0;
    char* iter_env = getenv("NCCL_STATS_ITERATION");
    if (iter_env) {
