#!/usr/bin/env python3
"""
P4 V3: CRUX Baseline — Job 1 (Static GPU Intensity Priority)
...
Job1: compute=50ms, comm=~85ms → I_1 ≈ 0.59
Job2: compute=50ms, comm=~85ms → I_2 ≈ 0.59

In this experiment, both jobs have similar GPU Intensity, so we assign
Job1 a slightly higher static priority (P4) vs Job2 (P3) to demonstrate
CRUX's static allocation behavior.

Launch:   python3 p4_job1_crux.py --mode crux
...
def main():
    ...
    if rank == 0:
        print(f"[Job1-CRUX] MODE={MODE}, Payload={PAYLOAD_MB}MB, "
              f"sleep={SLEEP_US}us")
    ...
    if rank == 0 and device is not None:
        ...
        print(f"[Job1-CRUX] GPU {device}: {torch.cuda.get_device_name(device)}, "
    ...
    if rank == 0:
        ...
        if solo_iters:
            ...
            print(f"[Job1-CRUX] Solo ({len(solo_iters)} iters): "
        ...
        if contested_iters:
            ...
            print(f"[Job1-CRUX] Contested ({len(contested_iters)} iters): "
                  f"avg_comm={avg_cont*1000:.1f}ms, avg_bw={avg_bw_cont:.2f} Gbps, "
                  f"slowdown={slowdown:.2f}x")

        print(f"[Job1-CRUX] Total wall time: {t_total_end - t_total_start:.1f}s")
