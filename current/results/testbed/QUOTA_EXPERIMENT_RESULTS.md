...
## 实验 C v2：CPU Epoch 模拟器多作业阶梯（2026-07-30）

> **目的**：把硬件验证从 2 作业扩到 5-6 作业、从单点扩到 5 个 regime，对标仿真 E1 梯子。
> **方法**：ibv verbs RDMA write 模拟器 + 独立守护进程（复用 SLOScheduler）+ Multi-QP 预创建
> **v2 改进**：迭代级 slowdown（s = (Tcomp+comm)/(Tcomp+Tcomm_solo)）、policy/eval c 拆分（c_policy=1.35, c_eval=1.5）、5 轮重复

### 场景设计

| 场景 | 作业数 | Premium | Standard | D scale | 定位 |
|------|--------|---------|----------|---------|------|
| S1_ample | 5 | 2 (J0, J1) | 3 (J2-J4) | ×0.6 | 充裕，验证"LongLiu 无代价" |
| S1_moderate | 5 | 2 | 3 | ×1.0 | 过渡，fair 开始伤 premium |
| S1_deep | 5 | 2 | 3 | ×1.3 | 深度稀缺，fair premium 明显违约 |
| S1_very_deep | 5 | 2 | 3 | ×2.0 | 极度稀缺，唯一 λ<1 的点 |
| S2_starvation | 6 | 3 (J0-J2, 8MB) | 3 (J3-J5, 4MB) | ×1.0 | 饥饿对照，有界 vs 无界退化 |
...
### S1 Premium Slowdown 跨 Regime 对比
...
**Premium Slowdown 对比（三臂）**：

| Job | Tier | Fair SD | LongLiu SD | Static SD | 最佳臂 |
|-----|------|---------|-----------|-----------|--------|
| J0 | P | **2.272±0.024** | 2.721±0.888 | 2.944±0.036 | Fair |
| J1 | P | **2.286±0.011** | 3.006±1.138 | 3.005±0.023 | Fair |
| J2 | P | 1.679±0.017 | **1.428±0.457** | 1.955±0.011 | LongLiu |

**Standard Slowdown 对比（三臂）**：
...
**S2 关键洞察**：LongLiu 将 J2（较小 φ 的 premium）的 SD 从 1.97（static）降到 1.43，展示了动态优先级的保护效果

### 铁律验证

| 铁律 | S1_ample | S1_moderate | S1_deep | S1_very_deep | S2_starvation |
|------|----------|-------------|---------|--------------|---------------|
| 1. 真实争用 | ✗ | ✗ | ✓ | ✗ | ✓ |
| 2. Fair 必败 | ✗ | ✗ | ✗ | ✗ | **✓ (P-SD=2.08)** |
| 3. LL 达标 | ✓ | ✓ | ✓ | ✓ | ✗ (结构性不可行) |

**问题**：S1 中铁律 1 和 2 不满足，原因是：
1. Tcomp 太短（~1ms），φ 实际 0.56 vs 设计 0.27
2. 相位互斥效应使争抢自消退
3. mlx5 不支持 per-priority QoS

### 关键发现

1. **LongLiu 在 moderate/deep 中保护 premium**（P-attn 优于 Static），但优势幅度有限
2. **LongLiu 的 standard 代价有界**（max SD ≤ 1.6），符合 Theorem 1
3. **S2 中 LongLiu 对 J2 的保护效果**：SD 从 1.97（static）或 1.68（fair）降到 1.43
4. **S2 Fair premium 全违约**（SD=2.08 > c_eval=1.5），铁律 2 首次在硬件上验证通过
5. **S1 中 Fair premium SD 不随稀缺加深**，与 E1 仿真定性不一致
6. **S2 premium jobs 因 RDMA 传输超时提前终止**，是极端争抢的预期行为

### 与 E1 仿真的定性对比

| 预期 | E1 仿真 | S1 硬件 | 一致？ |
|------|--------|--------|--------|
| Fair premium SD 随稀缺加深 | ✓ | ✗ | ✗ |
| LongLiu P-attn ≤ Static | ✓ | ✓ (moderate/deep) | ✓ |
| Ample regime 三臂收敛 | ✓ | ✓ | ✓ |
| LongLiu standard 代价有界 | ✓ | ✓ | ✓ |

### 数据归档

- 数据目录: `experiments_supplementary/03_exp_C_scale_ladder/data_v2/` (75 轮)
- 分析脚本: `experiments_supplementary/03_exp_C_scale_ladder/analysis/analyze_expC_v2.py`
- 完整报告: `experiments_supplementary/03_exp_C_scale_ladder/expC_v2_results.md`
- 场景定义: `experiments_supplementary/03_exp_C_scale_ladder/scenarios/scenarios_v2.json`
