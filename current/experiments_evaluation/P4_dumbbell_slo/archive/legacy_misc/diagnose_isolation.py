...
Setup:
  ...
  - Both run contested for 10 windows
...
class StaticPriorityScheduler:
    ...
    def update(self, actual_comm_time, data_size, window_comm_time=0.0):
...
def main():
    ...
    if rank == 0:
        ...
        print(f"[Diag-{JOB}] {NUM_ITERS} iters, {ITERS_PER_WINDOW}/window, "
    ...
    for window in range(NUM_WINDOWS):
        ...
        for i in range(ITERS_PER_WINDOW):
            ...
            if rank == 0 and i == 0:
                print(f"[Diag-{JOB}] window {window}: comm={comm_dur*1000:.1f}ms, "
    ...
    if rank == 0:
        ...
        with open(csv_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['iter', 'window', 'comm_dur_s', 'bw_gbps'])
            for r in results:
