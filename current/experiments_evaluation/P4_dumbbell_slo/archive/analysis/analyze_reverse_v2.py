...
Generates:
  - reverse_v2_comparison_window.csv : per-window comparison (both modes, both jobs)
  - reverse_v2_summary.txt          : human-readable summary with form description

Slowdown is recomputed correctly using per-window c_i:
  slowdown = avg_comm_per_iter / (c_i * T_target_per_iter)
  where T_target_per_iter = T_target_ms / 1000 / iters_per_window
...
TTARGET_A_PER_ITER_S = TTARGET_A_MS / 1000.0 / ITERS_PER_WINDOW
TTARGET_B_PER_ITER_S = TTARGET_B_MS / 1000.0 / ITERS_PER_WINDOW


def load_window_csv(path):
    """Load per-window CSV, return list of dicts with corrected slowdown."""
    rows = []
    ...
    with open(path) as f:
        for r in csv.DictReader(f):
            # Recompute slowdown correctly using per-window c_i
            avg_comm = float(r['avg_comm_s'])
            t_target_ms = float(r['t_target_ms'])
            c_i = float(r['c_i'])  # per-window c_i (V3: swaps at REVERSE_WINDOW)
            t_target_per_iter = t_target_ms / 1000.0 / ITERS_PER_WINDOW
...
def main():
    ...
    for job in ['A', 'B']:
        for mode in ['longliu', 'crux']:
            path = os.path.join(EXP_DIR, f'p4_job{job}_reverse_{mode}_rank0_window.csv')
            data[(job, mode)] = load_window_csv(path)

    # ============================================================
    # Write merged comparison CSV
    # ============================================================
    out_path = os.path.join(EXP_DIR, 'reverse_v2_comparison_window.csv')
    with open(out_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            'mode', 'job', 'window', 'phase', 'payload_mb', 'c_i',
        ...
        for (job, mode), rows in sorted(data.items()):
            for r in rows:
                writer.writerow([
                    mode, job, r['window'], r['phase'], r['payload_mb'], r['c_i'],
    ...
    summary_lines.append(f"  Job A (512MB): T_target = {TTARGET_A_MS}ms/window "
                         f"= {TTARGET_A_PER_ITER_S*1000:.1f}ms/iter")
    summary_lines.append(f"  Job B (512MB): T_target = {TTARGET_B_MS}ms/window "
                         f"= {TTARGET_B_PER_ITER_S*1000:.1f}ms/iter")
    summary_lines.append(f"SLO: c_i swaps at window {REVERSE_WINDOW}")
    summary_lines.append(f"  Phase 1 (windows 0-{REVERSE_WINDOW-1}): A c_i=1.6 (tight), B c_i=3.0 (loose)")
    summary_lines.append(f"  Phase 2 (windows {REVERSE_WINDOW}-14): A c_i=3.0 (loose), B c_i=1.6 (tight)")
    ...
    summary_lines.append(f"Reversal: window {REVERSE_WINDOW} (c_i swap, same payload 512MB)")
    summary_lines.append("")
    ...
    for mode in ['longliu', 'crux']:
        ...
        for job in ['A', 'B']:
            ...
            for phase_name, phase_rows in [('Phase 1', p1), ('Phase 2', p2)]:
                ...
                summary_lines.append(f"    {phase_name} (payload={payload}MB, {len(phase_rows)} windows):")
                ...
                summary_lines.append(f"      SLO met   = {slo_met_count}/{len(phase_rows)} windows")
    ...
    summary_lines.append("1. Priority trajectory (per window):")
    ...
    # π trajectory
    summary_lines.append("")
    summary_lines.append("2. π trajectory (per window):")
    summary_lines.append("   LongLiu v1(π):")
    for job in ['A', 'B']:
        rows = data[(job, 'longliu')]
        if rows:
    ...
    # Slowdown trajectory
    summary_lines.append("")
    summary_lines.append("3. Slowdown vs phase-specific c_i trajectory (per window):")
    ...
    summary_lines.append("")
    summary_lines.append("4. Per-window avg comm time (ms/iter):")
    for mode in ['longliu', 'crux']:
        label = "LongLiu v1(π)" if mode == "longliu" else "CRUX-static"
        summary_lines.append(f"   {label}:")
        for job in ['A', 'B']:
    ...
    for mode in ['longliu', 'crux']:
        ...
        for job in ['A', 'B']:
            ...
            if p2:
                ...
                summary_lines.append(f"   {label} Job {job}: {violated}/{len(p2)} windows violated")
