...
Generate per-window comparison data for plotting.

Output:
  - reverse_comparison_window.csv  (per-window summary, both modes, both jobs)
...
def load_window_csv(job, mode):
    """Load per-window CSV for a given job and mode."""
...
def analyze():
    ...
    for mode in modes:
        for job in jobs:
            data[(mode, job)] = {
                'window': load_window_csv(job, mode),
    ...
    for mode in modes:
        for job in jobs:
            for row in data[(mode, job)]['window']:
                pi_str = f"{float(row['pi']):+.4f}" if row['pi'] != 'nan' else 'nan'
                print(f"{mode:<8} {job:<4} {row['window']:<6} {row['phase']:<7} "
                      f"{row['sleep_us']:<6} "
    ...
    for mode in modes:
        for job in jobs:
            window_data = data[(mode, job)]['window']
            for phase in ['phase1', 'phase2']:
                phase_rows = [r for r in window_data if r['phase'] == phase]
                if not phase_rows:
                    continue
    ...
    for job in jobs:
        ...
        for mode in modes:
            ...
            p1_prios = [int(r['priority']) for r in window_data if r['phase'] == 'phase1']
            p2_prios = [int(r['priority']) for r in window_data if r['phase'] == 'phase2']
            ...
            p1_bws = [float(r['avg_bw_gbps']) for r in window_data if r['phase'] == 'phase1']
            p2_bws = [float(r['avg_bw_gbps']) for r in window_data if r['phase'] == 'phase2']
    ...
    combined_window_path = os.path.join(EXP_DIR, 'reverse_comparison_window.csv')
    with open(combined_window_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['mode', 'job', 'window', 'phase', 'sleep_us',
                         'avg_comm_ms', 'avg_bw_gbps', 'pi', 'priority', 'dscp'])
        for mode in modes:
            for job in jobs:
                for row in data[(mode, job)]['window']:
                    writer.writerow([
                        mode, job, row['window'], row['phase'], row['sleep_us'],
                        f"{float(row['avg_comm_s'])*1000:.2f}",
                        f"{float(row['avg_bw_gbps']):.4f}",
                        row['pi'], row['priority'], row['dscp']
                    ])
    print(f"\nCombined per-window CSV: {combined_window_path}")
    ...
    with open(combined_iter_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['mode', 'job', 'iter', 'window', 'sleep_us',
                         'comm_dur_ms', 'bw_gbps', 'phase'])
        for mode in modes:
            for job in jobs:
                for row in data[(mode, job)]['iter']:
                    writer.writerow([
                        mode, job, row['iter'], row['window'], row['sleep_us'],
