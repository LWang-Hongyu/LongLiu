...
def compute_slowdown(avg_comm_s, t_target_epoch_ms, c_i):
    ...
    where T_target_per_iter_s = t_target_epoch_ms / 1000 / ITERS_PER_WINDOW

    Violation when per-iter slowdown > 1.0, i.e., when avg_comm > c_i * T_target_per_iter
    Equivalently: ratio = (avg_comm_s * 1000) / (t_target_epoch_ms / ITERS_PER_WINDOW * c_i)
    """
    t_target_per_iter_ms = t_target_epoch_ms / ITERS_PER_WINDOW
...
def analyze():
    ...
    for scheduler in ["longliu", "crux"]:
        ...
        for job in ["jobA", "jobB"]:
            ...
            t_target_per_iter_ms = ttarget_ms / ITERS_PER_WINDOW
