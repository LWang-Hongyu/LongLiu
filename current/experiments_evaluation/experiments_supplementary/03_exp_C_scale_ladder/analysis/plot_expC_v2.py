...
Uses actual data from expC_v2_per_round.csv
...
SCENARIOS_FILE = EXP_C_DIR / "scenarios" / "scenarios_v2.json"
OUTPUT_DIR = SCRIPT_DIR


def load_data():
    """Load per-round data from CSV."""
    csv_file = OUTPUT_DIR / "expC_v2_per_round.csv"
    if not csv_file.exists():
        print(f"[ERROR] Data file not found: {csv_file}")
        print("Run analyze_expC_v2.py first.")
        sys.exit(1)
...
def main():
    ...
    plot_s1_pattn(df, OUTPUT_DIR)
    
    print("\n[plot] Generating S2 per-job slowdown chart...")
    plot_s2_perjob(df, OUTPUT_DIR)
    
    print("\n[plot] Generating S1 premium slowdown line chart...")
    plot_s1_premium_sd(df, OUTPUT_DIR)
