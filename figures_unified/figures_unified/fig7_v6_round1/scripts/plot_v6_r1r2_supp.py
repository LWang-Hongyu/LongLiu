#!/usr/bin/env python3
"""
Fig-7 (Evaluation Supplement): V6 round-1 + round-2 contested deployment.
...
Panel fig7a (trajectory): window-by-window slowdown averaged over the two
  rounds (mean line + round min/max band), LongLiu vs CRUX; SLO-tight window
  range shaded.
Panel fig7b (summary): per (bg × SLO-tightness) mean slowdown bars
  (mean over 2 rounds × 2 jobs, error bar = min–max, window 0 excluded).

Data:
  P4_dumbbell_slo/data_v6_bg{15,30}_round{1,2}/
    p4_job{A,B}_v6_round{R}_{LLthenCX|CXthenLL}_{longliu,crux}_rank0_window.csv
"""
import csv, os
import numpy as np
import matplotlib
...
OUT_DIR = os.path.join(os.environ.get('TEMP', FIG_ROOT), 'fig7_v6_round1_figs')
DATA = os.path.join(FIG_ROOT, 'data')
...
def load_window(bg, rnd, job, mode):
    """Return (windows, slowdown) arrays for one CSV."""
    f = os.path.join(DATA, f'data_v6_bg{bg}_round{rnd}',
                     f'p4_job{job}_v6_round{rnd}_{ORDER_OF[rnd]}_{mode}_rank0_window.csv')
    ws, ss = [], []
    with open(f) as fh:
...
def load_window_across_rounds(bg, job, mode):
    """Return per-window (mean, min, max) over the two rounds (windows 0-14)."""
    w = None
    cols = {m: [] for m in ('mean', 'min', 'max')}
    for rnd in ROUNDS:
        w, s = load_window(bg, rnd, job, mode)
        cols['mean'].append(s)
    arr = np.array(cols['mean'])  # (n_rounds, n_windows)
    return w, arr.mean(axis=0), arr.min(axis=0), arr.max(axis=0)


def job_phase_mean(bg, rnd, job, mode, tight):
...
def draw_panel(ax, bg, job, title, xlabel, ylabel):
    ...
    for mode, style in (('longliu', LL_STYLE), ('crux', CX_STYLE)):
        w, m, lo, hi = load_window_across_rounds(bg, job, mode)
        ax.fill_between(w, lo, hi, color=style['color'], alpha=BAND_ALPHA, zorder=1)
        ax.plot(w, m, **style)
...
def main():
    ...
    # ── Fig 7a: window-level trajectories (mean over 2 rounds) ──
    fig, axes = plt.subplots(2, 2, figsize=(FIG_W, FIG_H), sharex=True, sharey=True)
    fig.subplots_adjust(left=0.075, right=0.985, bottom=0.16, top=0.75,
    ...
    for bg, job in specs:
        for mode in ('longliu', 'crux'):
            _, m, hi, _ = load_window_across_rounds(bg, job, mode)
    ...
    handles = [
        plt.Line2D([], [], color=LL_COLOR, lw=1.6, ls='-', marker='s', ms=3.5,
                   markerfacecolor=LL_COLOR, label='LongLiu (P6), mean ± round range'),
        plt.Line2D([], [], color=CX_COLOR, lw=1.6, ls='--', marker='o', ms=3.5,
                   markerfacecolor=CX_COLOR, label='CRUX (P3), mean ± round range'),
    ...
    fig.savefig(os.path.join(OUT_DIR, 'fig7_v6_r1r2_trajectory.pdf'),
                bbox_inches='tight', pad_inches=0.02)
    fig.savefig(os.path.join(OUT_DIR, 'fig7_v6_r1r2_trajectory_600.png'),
                dpi=600, bbox_inches='tight', pad_inches=0.02)
    print('[OK] fig7_v6_r1r2_trajectory')

    # ── Fig 7b: phase-summary bars (2 rounds × 2 jobs) ─────────
    ...
    for _, bg, tight in groups:
        v_ll = [job_phase_mean(bg, r, j, 'longliu', tight) for r in ROUNDS for j in 'AB']
        v_cx = [job_phase_mean(bg, r, j, 'crux', tight) for r in ROUNDS for j in 'AB']
    ...
    ax2.set_xticklabels([f'{g}\nSLO-{"tight" if t else "loose"}' for g, _, t in groups],
                        fontsize=11)
    ax2.set_ylabel('Mean slowdown (×)', fontsize=12)
    ax2.set_ylim(0, max(max(ll_hi), max(cx_hi)) * 1.15)
    ax2.tick_params(axis='y', labelsize=10)
    ax2.grid(True, axis='y', alpha=0.30, linewidth=0.4, linestyle=':')
    ax2.legend(loc='upper right', fontsize=11, ncol=1, framealpha=0.9)
    fig2.savefig(os.path.join(OUT_DIR, 'fig7_v6_r1r2_summary.pdf'),
                 bbox_inches='tight', pad_inches=0.02)
    fig2.savefig(os.path.join(OUT_DIR, 'fig7_v6_r1r2_summary_600.png'),
                 dpi=600, bbox_inches='tight', pad_inches=0.02)
    print('[OK] fig7_v6_r1r2_summary')

    # ── console summary (cross-check numbers for the supplement) ──
    print('\n  per-round means (LL / CX, mean over 2 jobs):')
    for rnd in ROUNDS:
        for bg in (15, 30):
            for tight in (True, False):
                v_ll = [job_phase_mean(bg, rnd, j, 'longliu', tight) for j in 'AB']
                v_cx = [job_phase_mean(bg, rnd, j, 'crux', tight) for j in 'AB']
                lm, cm = np.mean(v_ll), np.mean(v_cx)
                tag = 'tight' if tight else 'loose'
                print(f'    R{rnd} bg{bg} {tag:5s}: LL={lm:.2f}  CX={cm:.2f}  '
                      f'delta={(cm - lm) / cm * 100:+.1f}% (CX-relative)')
    print('\n  combined (2 rounds × 2 jobs):')
    for gi, (g, bg, tight) in enumerate(groups):
        tag = 'tight' if tight else 'loose'
        print(f'    {g} {tag:5s}: LL={ll_means[gi]:.2f}  CX={cx_means[gi]:.2f}  '
              f'delta={(cx_means[gi] - ll_means[gi]) / cx_means[gi] * 100:+.1f}% '
              f'(CX-relative)')
