...
策略：LongLiu、CRUX
预登记：
  - LongLiu W1: P-attn=100%, S-cont-cap≈0.74
  - LongLiu W3: P-attn=100%, S-cont-cap≈0.62, starv=0
  - LongLiu W2: 闭式无瞬态，1-2 epoch 内收敛
  - CRUX W1: P-attn≈87.5%±
...
with open(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), "config.yaml")) as f:
...
SEEDS = [0, 1, 2, 3, 4]
SEED = SEEDS[0]  # run_single 默认种子（保持旧调用兼容）
...
def run_single(policy_name: str, workload_raw=None, spine_bw_gbps=800,
    ...
    elif policy_name == "LL-S":
        policy = LongLiu(use_dynamic_T_target=False)
    elif policy_name == "LongLiu":
...
def aggregate_seeds(runs):
    """将多种子 run_meta 列表聚合为 mean 字典。"""
    if not runs:
        return None
    agg = {}
    for win in ("w1", "w2", "w3"):
        agg[win] = {
            "p_attn": float(np.mean([r[win]["p_attn"] for r in runs])),
            "p_attn_std": float(np.std([r[win]["p_attn"] for r in runs])),
            "s_cont_cap": float(np.mean([r[win]["s_cont_cap"] for r in runs])),
            "s_cont_cap_std": float(np.std([r[win]["s_cont_cap"] for r in runs])),
            "n_premium": runs[0][win]["n_premium"],
            "n_premium_attn": runs[0][win]["n_premium_attn"],
            "n_standard_with_data": runs[0][win]["n_standard_with_data"],
        }
    agg["starv_pre_swap"] = max(r["starv_pre_swap"] for r in runs)
    agg["starv_post_swap"] = max(r["starv_post_swap"] for r in runs)
    agg["n_seeds"] = len(runs)
    agg["per_seed"] = [
        {
            "seed": r["seed"],
            "w1_p_attn": r["w1"]["p_attn"],
            "w2_p_attn": r["w2"]["p_attn"],
            "w3_p_attn": r["w3"]["p_attn"],
            "w1_s_cont_cap": r["w1"]["s_cont_cap"],
            "w3_s_cont_cap": r["w3"]["s_cont_cap"],
...
def main():
    """Run both E3 (control arm) and E3' (kill arm) swap experiments (5 seeds)."""
    ...
    for cfg in configs:
        ...
        runs_by_policy = {pn: [] for pn in POLICIES}
        for pn in POLICIES:
            for seed in SEEDS:
                print(f"[{pn} s{seed}] ", end="", flush=True)
                try:
                    r = run_single(pn, workload_raw=cfg["workload"],
                                   spine_bw_gbps=cfg["spine_bw"],
                                   tag_prefix=cfg["tag_prefix"], seed=seed)
                    runs_by_policy[pn].append(r)
                    print(f"W1 P-attn={r['w1']['p_attn']*100:.1f}% "
                          f"W3 P-attn={r['w3']['p_attn']*100:.1f}% "
            ...
            agg = aggregate_seeds(runs_by_policy[pn])
            if agg:
                w1 = agg["w1"]
                w3 = agg["w3"]
                print(f"  → W1 P-attn={w1['p_attn']*100:.1f}±{w1['p_attn_std']*100:.1f}% "
                      f"| W3 P-attn={w3['p_attn']*100:.1f}±{w3['p_attn_std']*100:.1f}% "
                      f"S-cap={w3['s_cont_cap']:.3f}±{w3['s_cont_cap_std']:.3f} "
                      f"| starv_pre={agg['starv_pre_swap']} starv_post={agg['starv_post_swap']}")

        results = {pn: aggregate_seeds(runs_by_policy[pn]) for pn in POLICIES}
        ...
        if cfg["tag_prefix"] == "e3_swap":
            ...
            if results["LongLiu"]:
                ...
                w1, w3 = r["w1"], r["w3"]
                checks = [
                    ("LongLiu W1 P-attn=100%", w1["p_attn"], 1.0, 0.02),
                    ("LongLiu W3 P-attn=100%", w3["p_attn"], 1.0, 0.02),
                    ("LongLiu W3 starv=0", 1.0 - float(r["starv_post_swap"] > 0), 1.0, 0.0),
                    ("LongLiu W3 S-cont-cap≥0.62 (lower bound)", w3["s_cont_cap"], 0.62,
            ...
            if results["CRUX"]:
                w3 = results["CRUX"]["w3"]
                print(f"  [INFO] CRUX W3 P-attn={w3['p_attn']*100:.1f}±{w3['p_attn_std']*100:.1f}% "
            ...
            if results["CRUX"]:
                ...
                # Key check: CRUX W3 P-attn ≪ LongLiu
                if results["LongLiu"]:
                    longliu_w3_pattn = results["LongLiu"]["w3"]["p_attn"]
                    crux_w3_pattn = results["CRUX"]["w3"]["p_attn"]
                    gap = (longliu_w3_pattn - crux_w3_pattn) * 100
                    ok = gap >= 10.0
                    flag = "PASS" if ok else "FAIL"
                    if not ok:
                        all_pass = False
                    print(f"  [{flag}] CRUX W3 P-attn ≪ LongLiu (gap≥10pp): "
                          f"LongLiu={longliu_w3_pattn*100:.1f}% CRUX={crux_w3_pattn*100:.1f}% gap={gap:.1f}pp")
        ...
        for pn, r in results.items():
            if r is None:
                continue
            summary["results"][pn] = {
                "w1": {k: v for k, v in r["w1"].items() if k != "per_seed"},
                "w2": {k: v for k, v in r["w2"].items() if k != "per_seed"},
                "w3": {k: v for k, v in r["w3"].items() if k != "per_seed"},
                "starv_pre_swap": r["starv_pre_swap"],
                "starv_post_swap": r["starv_post_swap"],
                "n_seeds": r["n_seeds"],
                "per_seed": r["per_seed"],
